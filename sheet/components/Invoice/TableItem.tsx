...
const ItemCell: React.FC<React.PropsWithChildren<{ className: string }>> = ({
  ...
  return (
    <th className={`text-center flex items-center justify-center font-medium text-sm text-gray-400 ${className}`}>
      {children}
    </th>
  );
};
...
export default function TableItem(item: AccountData) {
  return (
    <tr className="flex w-full border-b border-x border-gray-200 hover:bg-gray-100 py-2">
      ...
      <ItemCell className="basis-[12%]">{item.totalBalance}</ItemCell>
      <ItemCell className="basis-[15%]">{formatDate(item.issueDate)}</ItemCell>
      <ItemCell className="basis-[14%]">
        {renderBalanceContent(false, item.hasPaid, item.balance.toString())}
      </ItemCell>
